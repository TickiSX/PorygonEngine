...
public:
	...
	/**	 * @brief Prepara subsistemas previos al render.	 * @return `S_OK` si la aplicacion queda lista para continuar la inicializacion.	 */
	HRESULT awake();

	/**	 * @brief Ejecuta el bucle principal de la aplicacion.	 * @param hInst Instancia Win32 actual.	 * @param nCmdShow Modo inicial de visualizacion de la ventana.	 * @return Codigo de salida del proceso.	 */
	int run(HINSTANCE hInst, int nCmdShow);

	/**	 * @brief Inicializa recursos graficos, escena, materiales y renderer.	 * @return `S_OK` cuando todos los recursos base quedan listos.	 */
	HRESULT init();

	/**	 * @brief Ejecuta la logica por frame y sincroniza GUI, camara y escena.	 * @param deltaTime Tiempo transcurrido desde el frame anterior.	 */
	...
	/**	 * @brief Reconstuye recursos dependientes de la resolucion principal.	 * @param newW Nuevo ancho del area cliente.	 * @param newH Nuevo alto del area cliente.	 */
	...
	/**	 * @brief Serializa la escena actual a disco.	 * @param path Ruta de salida del archivo `.wvscene`.	 * @return `true` si la escena se guarda correctamente.	 */
	bool saveScene(const std::string& path);

	/**	 * @brief Carga una escena serializada previamente.	 * @param path Ruta del archivo `.wvscene`.	 * @return `true` si el contenido se pudo leer y aplicar.	 */
...
private:
	...
	CBPerFrame							m_constantBufferStruct;
