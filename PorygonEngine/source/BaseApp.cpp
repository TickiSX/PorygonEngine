...
BaseApp::init() {
    ...
    // --- RUTA RAIZ ABSOLUTA apuntando a tu carpeta bin/ de PorygonEngine ---
    ...
    std::array<std::string, 6> faces = {
        ROOT_PATH + "Skybox/cubemap_0.png",
        ROOT_PATH + "Skybox/cubemap_1.png",
        ROOT_PATH + "Skybox/cubemap_2.png",
        ROOT_PATH + "Skybox/cubemap_3.png",
        ROOT_PATH + "Skybox/cubemap_4.png",
        ROOT_PATH + "Skybox/cubemap_5.png"
    };
    ...
    if (!m_cyberGun.isNull()) {
        ...
        if (!m_model || !m_model->load(ROOT_PATH + "Assets/MA5C.fbx")) {
            ERROR("Main", "InitDevice", "Failed to load MA5C model.");
            return E_FAIL;
        }

        hr = m_AlbedoSRV.init(m_device, ROOT_PATH + "Assets/MA5C_2K_Color", ExtensionType::PNG);
        if (FAILED(hr)) return hr;

        hr = m_MetallicSRV.init(m_device, ROOT_PATH + "Assets/MA5C_2K_Metallic", ExtensionType::PNG);
        if (FAILED(hr)) return hr;

        hr = m_RoughnessSRV.init(m_device, ROOT_PATH + "Assets/MA5C_2K_Glossiness", ExtensionType::PNG);
        if (FAILED(hr)) return hr;

        hr = m_AOSRV.init(m_device, ROOT_PATH + "Assets/MA5C_Mg_2K_Color", ExtensionType::PNG);
        if (FAILED(hr)) return hr;

        hr = m_NormalSRV.init(m_device, ROOT_PATH + "Assets/MA5C_2K_NormalGL", ExtensionType::PNG);
        if (FAILED(hr)) return hr;

        // Soporte Emisivo añadido
        HRESULT emissiveHr = m_EmissiveSRV.init(m_device, ROOT_PATH + "Assets/MA5C_Emissive", ExtensionType::PNG);
        if (FAILED(emissiveHr)) {
            MESSAGE("Main", "InitDevice", "MA5C emissive texture not found. Continuing without emissive map.");
        }
...
BaseApp::destroy() {
    ...
    m_AOSRV.destroy();
    m_EmissiveSRV.destroy(); // Destrucción del mapa emisivo
...
BaseApp::onResize(unsigned int newW, unsigned int newH) {
    ...
    // 3) Libera recursos dependientes del tamaño
    m_renderTargetView.destroy();
    m_depthStencilView.destroy();
    m_depthStencil.destroy();
    m_backBuffer.destroy();
