...
void Model3D::ProcessFBXMesh(FbxNode* node) {
	...
	for (int p = 0; p < mesh->GetPolygonCount(); ++p)
	{
		...
		for (int v = 0; v < polySize; ++v)
		{
			...
			// Si la normal viene vacía, le damos un valor seguro hacia enfrente
			if (out.Normal.x == 0 && out.Normal.y == 0 && out.Normal.z == 0) {
			...
			if (tanElem) {
				...
				out.Tangent = { 1.0f, 0.0f, 0.0f }; // Valor seguro
			}
			...
			if (binElem) {
				...
				out.Bitangent = { 0.0f, 1.0f, 0.0f }; // Valor seguro
			}
		...
		// Triangula en abanico (CW por defecto)
