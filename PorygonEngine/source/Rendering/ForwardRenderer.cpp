...
void ForwardRenderer::updatePerFrame(const Camera& camera, const RenderScene& scene, DeviceContext& deviceContext) {
	// 1. Calculamos matrices de sombras
	updateLightMatrices(camera, scene);

	// 2. Pasamos las matrices de la cámara
	...
	// 3. Extraemos la luz de la escena y la pasamos al buffer
...
void ForwardRenderer::buildQueues(RenderScene& scene, const Camera& camera) {
	...
	std::sort(m_opaqueQueue.begin(), m_opaqueQueue.end(), [](const RenderObject* lhs, const RenderObject* rhs) {
	...
	std::sort(m_transparentQueue.begin(), m_transparentQueue.end(), [](const RenderObject* lhs, const RenderObject* rhs) {
		return lhs->distanceToCamera > rhs->distanceToCamera;
		});
}
